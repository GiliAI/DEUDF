...
class SineLayer(nn.Module):
    ...
    def init_weights(self):
        with torch.no_grad():
            if self.is_first:
                ...
                # self.linear.weight.uniform_(-np.sqrt(6 / self.dim)/30,
                #                             np.sqrt(6 / self.dim)/30)
    ...
    def forward(self, input):
        ...
        #     return torch.sin(self.omega_0 * self.linear(input))
        # else:
        #     return torch.sin(self.omega_0 * 30 * self.linear(input))
...
class SirenModule(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers,
                 ...
                 first_omega_0=30, hidden_omega=30.):
        super().__init__()
        ...
        for i in range(hidden_layers):
            if(not isinstance(hidden_omega,float) and not isinstance(hidden_omega,int)):           
                hidden_omega_0 = hidden_omega[i+1]
            self.net.append(SineLayer(hidden_features, hidden_features,
                                        is_first=False, omega_0=hidden_omega_0))
        if(not isinstance(hidden_omega,float) and not isinstance(hidden_omega,int)):           
            hidden_omega_0 = hidden_omega[-1]
        if outermost_linear:
...
class Siren(Network):
    ...
    def _initialize(self):
        if len(self.omegas)>0:
            omega0 = self.omegas[0]
            hidden_omega = self.omegas
        else:
            omega0 = self.omega
            hidden_omega = self.omega
        ...
        # for i in range(len(self._module.net)):
        #     setattr(self,"layer"+str(i),self._module.net[i])
