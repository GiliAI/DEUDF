...
def process_data(data_dir, dataname):
    if os.path.exists(os.path.join(data_dir, 'input', dataname) + '.ply'):
        pointcloud = trimesh.load(os.path.join(data_dir, 'input', dataname) + '.ply')
        if isinstance(pointcloud, trimesh.PointCloud):
            if pointcloud.vertices.shape[0]>1000000:
                new_pc = np.array(pointcloud.vertices)
                idx = np.random.choice(new_pc.shape[0], 100000, replace=False)
                pointcloud = new_pc[idx]
            else:
                pointcloud = pointcloud.vertices
        else:
            pointcloud = pointcloud.sample(1000000)
    ...
    QUERY_EACH = 10000000 // POINT_NUM_GT
    ...
    for i in range(QUERY_EACH):
        ...
        print(scale)
        print(scale * np.expand_dims(sigmas, -1).max())
        ...
        for j in range(tt.shape[0]):
            nearest_idx = ptree.query(tt[j])[1]
    ...
    sample_n2ear = np.asarray(sample_near)
