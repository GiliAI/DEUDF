    ...
    def train(self):
        ...
        for iter_i in tqdm(range(self.iter_step, self.step2_maxiter)):
            ...
            if self.iter_step % self.report_freq == 0:
                print_log('iter:{:8>d} cd_l1 = {} lr={}'.format(self.iter_step, loss_cd,
            ...
            if self.iter_step == self.step2_maxiter:
                ...
                evaluate = Evaluator(self.dataname, lambda pts: self.udf_network.udf(pts),
                                     conf_path=self.conf_path)
                try:
                    evaluate.evaluate()
                except IndexError:
                    with open("error_cut.txt", "a") as f:
                        f.write(args.dataname+"\n")
