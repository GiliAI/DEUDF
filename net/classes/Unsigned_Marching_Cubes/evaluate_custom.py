    ...
    for xi, xs in enumerate(X):
        for yi, ys in enumerate(Y):
            for zi, zs in enumerate(Z):
                # s_g = time.time()
                # print("start Extracted {},{},{} in {}s".format(xi,yi,zi,s_g))
                ...
                # e_g = time.time()
                # print("finished Extracted {},{},{} in {}s".format(xi,yi,zi,e_g-s_g))
...
class Evaluator:
    def __init__(self, dataname,query_func, conf_path="confs/base.conf", bound_min=None,bound_max=None):
        ...
        f = open(os. path.dirname (os. path.abspath (__file__))+"/"+self.conf_path)
        conf_text = f.read()
        f.close()
    ...
    def evaluate(self):
        ...
        min_face_area = torch.mean(ss_value).detach()/4
        ...
        for it in range(self.max_iter):
            ...
            s_value = calculate_ss(xyz, mesh.faces)

            s_loss = torch.log(1 + torch.exp(min_face_area - s_value))
