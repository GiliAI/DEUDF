...
class Evaluator:
    def __init__(self, args, conf_path, bounds=None, udf_network=None,total_size=None, centers=None):
        ...
        if self.bounds is None:
            ...
            self.total_size = self.dataset.total_size
            self.centers = self.dataset.centers
    ...
    def evaluate(self):
        ...
        if (not self.use_exist_mesh) or ( not os.path.exists(out_dir + '/MC_mesh.obj')):
            ...
            bmin = self.centers-0.3
            bmax = self.centers+0.3
            print(bmin)
            print(bmax)
            u, g = extract_fields(bmin, bmax, self.resolution, query_func, grad_func)
            print(u.shape)
            # g[u[:,:,:]>0.02] = (0,0,0)
