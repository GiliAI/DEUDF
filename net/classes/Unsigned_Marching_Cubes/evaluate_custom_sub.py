...
def face_calculation(mesh, equal_weight=True):
    """
    Calculate a sparse matrix for laplacian operations.
    Parameters
    -------------
    ...
    # create the sparse matrix
    matrix = coo_matrix((data, (row, col)),
                        shape=(neighbors.shape[0],vertices.shape[0]))
    values = matrix.data
    indices = np.vstack((matrix.row, matrix.col))
...
def get_abc(vertices, faces):
    fvs = vertices[faces]
    eps = 1e-10
    sub_a = fvs[:, 0, :] - fvs[:, 1, :] + eps
    sub_b = fvs[:, 1, :] - fvs[:, 2, :] + eps
    sub_c = fvs[:, 0, :] - fvs[:, 2, :] + eps
...
def calculate_edge_len_weight(xyz, neighbors):
    xyz = xyz.detach()
    result = []
    mask = neighbors>0
    for i in range(neighbors.shape[1]):
        re = torch.norm(xyz - xyz[neighbors[:,i]], dim=1).unsqueeze(1)
        result.append(re)
...
class Evaluator:
    ...
    def evaluate(self):
        # if os.path.exists(self.out_dir + '/' + '{}.ply'.format(self.dataname)):
        #     os.remove(self.out_dir + '/' + '{}.ply'.format(self.dataname))
        ...
        self.generate_pointcloud(self.mesh,1000000)
        ...
        neighbors = neighbors2tensor(self.mesh)[0]


        num_samples = xyz.shape[0]
        mid_head = 0
        # lap_weight = self.laplacian_weight*torch.ones(num_samples).cuda()
        for it in range(self.max_iter):
            # print(it)
            # if it % (self.max_iter//4) == 0:
            #     w_l = w_l*2

            if it <= self.normal_step and it % 100 ==0 and it>=100:
                self.mesh.vertices = xyz.detach().cpu().numpy()
                self.subvision()

                self.mesh.export(self.out_dir + '/test_{}.ply'.format(it))
                xyz = torch.from_numpy(self.mesh.vertices.astype(np.float32)).cuda()
                xyz.requires_grad = True
                # set optimizer to xyz
                ...
                # else:
                #     self.optimizer = torch.optim.Adam([xyz])
                laplacian_op = laplacian_calculation(self.mesh, equal_weight=False).cuda()
                normals = torch.FloatTensor(self.mesh.face_normals).cuda()
            ...
            while head< num_samples:
                ...
                df_loss = df.mean()
                loss = df_loss
                edge_lens = torch.norm(xyz[self.mesh.edges][:,0]-xyz[self.mesh.edges][:,1],dim=1)
                ...
                #     edg_loss = cir_loss(xyz, origin_edg,faces=self.mesh.faces)
                # else:
                ...
                if self.normal_step is not None and it < self.normal_step:
                    ...
                    edgelenght_weight = calculate_edge_len_weight(xyz, neighbors)
                    print(edgelenght_weight.min())
                    laplacian_loss = 1 * torch.sum(lap_v, dim=1)
                    laplacian_loss = laplacian_loss[head: min(head + self.max_batch, num_samples)]
                    laplacian_loss = self.laplacian_weight * laplacian_loss.mean()
                    loss = loss + laplacian_loss
                    # distance_loss = torch.norm(origin_xyz - xyz[:origin_xyz.shape[0]], dim=1)
                    # distance_loss[distance_loss < self.max_dist_threshold * self.threshold] = 0
                    # distance_loss = distance_loss.mean()
                    # loss = loss + distance_loss
                else:
                    pass

                    # offset = sample_subset - origin_points[head: min(head + self.max_batch, num_samples)]
                    # # offset = torch(offset,dim=-1)
                    # normal_loss =  torch.norm(torch.cross(offset,normals[head: min(head + self.max_batch, num_samples)],dim=-1),dim=-1)
                    # normal_loss = normal_loss.mean()
                    # loss += 1*normal_loss


                    offset = mid_points[mid_head: min(mid_head + self.max_batch, mid_points.shape[0])] - origin_points[mid_head: min(mid_head + self.max_batch, mid_points.shape[0])]
                    # offset = torch(offset,dim=-1)
                    normal_loss =  torch.norm(torch.cross(offset,normals[mid_head: min(mid_head + self.max_batch, mid_points.shape[0])],dim=-1),dim=-1)
                    normal_loss = normal_loss.mean()
                    loss += 0.5*normal_loss
                    # print(normal_loss)
                    # new_mid = get_mid(xyz,self.mesh.faces)
            ...
            if (it+1) % self.save_iter == 0:
                ...
                self.mesh.export(self.out_dir + '/{}_{}_Optimize_{}_sub.ply'.format(self.dataname,it,str(self.threshold)))
        # self.mesh.apply_scale(self.total_size)
        final_mesh = trimesh.load_mesh(self.out_dir + '/{}_{}_Optimize_{}_sub.ply'.format(self.dataname,self.max_iter-1,str(self.threshold)))
        # self.mesh.apply_translation(self.centers)
        if self.is_cut == 1:
            ...
            if final_mesh_cuple is not None:
                ...
                final_mesh_1.export(self.out_dir + '/' + '{}-0_{}_sub.ply'.format(self.dataname,str(self.threshold)))
                final_mesh_2.export(self.out_dir + '/' + '{}-1_{}_sub.ply'.format(self.dataname,str(self.threshold)))
                ...
                final_mesh.export(self.out_dir + '/' + '{}_{}_sub.ply'.format(self.dataname,str(self.threshold)))
            else:
    ...
    def subvision(self,sub_threshould=0.001):
        # weights = lap_weight.cpu().numpy()
        xyz = torch.from_numpy(self.mesh.vertices.astype(np.float32)).cuda()
        num_samples = xyz.shape[0]
        head = 0
        vertices_df = torch.zeros((0, 1))
        while head < num_samples:
            sample_subset = xyz[head: min(head + self.max_batch, num_samples)]
            df = self.query_func(sample_subset)
            # print("cal complete")
            ...
            # print(laplacian_loss)
            # w = 0.5 * (math.cos((it) / (self.max_iter) * math.pi/2) + 0.2)
            vertices_df = torch.vstack((vertices_df, df.detach()))
            head+=self.max_batch
        mid_point = get_mid(xyz,self.mesh.faces)
        num_samples = mid_point.shape[0]
        head = 0
        mid_vertices_df = np.zeros((0, 1))
        while head < num_samples:
            sample_subset = mid_point[head: min(head + self.max_batch, num_samples)]
            df = self.query_func(sample_subset)
            # print("cal complete")
            # if self.use_edge_loss:
            #     edg_loss = cir_loss(xyz, origin_edg,faces=mesh.faces)
            ...
            # print(laplacian_loss)
            # w = 0.5 * (math.cos((it) / (self.max_iter) * math.pi/2) + 0.2)
            mid_vertices_df = np.vstack((mid_vertices_df, df.detach().cpu().numpy()))
            head += self.max_batch
        lap_op = face_calculation(self.mesh)
        lap_df = torch.sparse.mm(lap_op,vertices_df.cpu())
        lap_df = lap_df.numpy()
        sub_df =np.abs(mid_vertices_df-lap_df).flatten()
        sub_face_idxs = np.where(sub_df > sub_threshould)[0]
        faces = np.asarray(self.mesh.faces)
        vertices = np.asarray(self.mesh.vertices)
        face_mask = np.zeros(len(faces), dtype=bool)
        face_mask[sub_face_idxs] = True
        ...
        refine_face_unshared = []
        for i in range(len(faces)):
            refine_face_unshared.append([])

        refine_face_mid_idxs = []
        for i in range(len(faces)):
        ...
        for id in refine_1:
            ...
            refine_faces.append([p1,mid_point_1_idx,p3])
            refine_faces.append([mid_point_1_idx, p2, p3])
        ...
        for id in refine_2:
            ...
            if p4 == p1:
                ...
                refine_faces.append([p1,mid_point_1_idx,p3])
                refine_faces.append([mid_point_1_idx, mid_point_2_idx, p3])
                refine_faces.append([mid_point_1_idx, p2, mid_point_2_idx])
            else:
                refine_faces.append([p1, mid_point_1_idx, mid_point_2_idx])
                refine_faces.append([mid_point_1_idx, p3,  mid_point_2_idx])
                refine_faces.append([mid_point_1_idx, p2, p3])
        ...
        for id in refine_3:
            ...
            refine_faces.append([mid_point_1_idx,mid_point_2_idx,mid_point_3_idx])
            refine_faces.append([p3, mid_point_3_idx,mid_point_2_idx])
            refine_faces.append([p4, mid_point_1_idx,mid_point_3_idx])
            refine_faces.append([p5, mid_point_2_idx, mid_point_1_idx])
        refine_faces = np.array(refine_faces)
        ...
        self.mesh = trimesh.Trimesh(new_vertices,new_faces)
