...
class Runner:
    ...
    def extract_mesh(self, resolution=64, threshold=0.0, point_gt=None, iter_step=0, logger=None):
        ...
        mesh = extract_geometry(bound_min, bound_max, resolution=resolution, npz_path=os.path.join(self.base_exp_dir,"field_0.005.npz"), \
                out_dir=out_dir, iter_step=iter_step, dataname=self.dataname, logger=logger)
        mesh.process()
        mesh.export(out_dir+'/'+'60000_mesh.obj')
