...
def generate_model(file_dir,file_name,assign_materials=True):
    ...
    ob.rotation_euler=(radians(90),radians(0),radians(180))
    ...
    alpha = 0.1
    transmission = 0.5
    bt.setMat_balloon(ob, bt.colorObj((179.0/255, 190.0/255, 203.0/255, 1), 0.5, 1.5, 1.0, 0.0, 0.0), alpha)
...
if __name__ == "__main__":
    in_floder = "D:\\render_data\\all_data_for_render\\water_tight\\"
    ...
    for obj_name in os.listdir(in_floder):
        ...
        # H = 1.2  # height of the circle camera path
        # lookAtPos = (0, 0, 0.2)  # look at position
        R = 4  # radius of the circle camera path
        H = 1.2  # height of the circle camera path
        lookAtPos = (0, 0, 0.6)  # look at position
        startAngle = 210  # camera starting angle from positive x-axis
        focalLength = 45
