...
def generate_model(file_dir,file_name,assign_materials=True):
    ...
    ob.rotation_euler=(radians(0),radians(0),radians(0))
    # ob.scale = (0.7, 0.7, 0.7)
    # ob.rotation_euler=(radians(-128),radians(0),radians(180))
    ob.location=(-0.0      ,-0.0      ,0.291391     )
    ...
    # ptColor = bt.colorObj(bt.colorObj((179.0/255, 190.0/255, 203.0/255, 1), 0.5, 1.3, 1.0, 0.0, 0.0))
    # ptSize = 0.014
    # bt.setMat_pointCloud(ob, ptColor, ptSize)
    idx = int(file_name.split("_")[-1][:-4])
    print(idx)
    x_idx = idx//(8*8)
    y_idx = (idx-8*8*x_idx)//8
    z_idx = (idx-8*8*x_idx-8*y_idx)
    print(x_idx, y_idx, z_idx)
    x_idx = x_idx%2
    y_idx = y_idx%2
    z_idx = z_idx%2
    print(x_idx,y_idx,z_idx)
    color = render_color[x_idx][y_idx][z_idx]
...
if __name__ == "__main__":
    reset_scene()
    in_floder = "D:\\Projects\\CXH\\Unsigned_Marching_Cubes\\postprocess\\sep_data\\"
    ...
    ## set light
    lightAngle = (-10.3141,18.3582, -59.2143)
    strength = 3
    shadowSoftness = 0.3
    ...
    ## set ambient light
    bt.setLight_ambient(color=(0.1, 0.1, 0.1, 0.8))
    ...
    H = 0.8  # height of the circle camera path
    lookAtPos = (0, 0, 0.1)  # look at position
    startAngle = 70  # camera starting angle from positive x-axis
    focalLength = 45
    ...
    outputHeader = os.path.join(folder, "big_" + in_floder.split("\\")[-2] + '_')
    print(outputHeader)
