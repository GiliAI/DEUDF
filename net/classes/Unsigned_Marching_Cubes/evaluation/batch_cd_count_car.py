...
exp_root = "../experiment/outs_car_capudf"
...
def batch_evaluate(name,conf):
    ...
    #     return
    result_path_1 = os.path.join(exp_root,name,"mesh","{}_{}.ply".format(name,str(r)))
    # result_path_2 = os.path.join(exp_root, name, "mesh", "{}-1_0.0075_sub.ply".format(name))
    ...
    # capudf_path = os.path.join(exp_root,name,"mesh", "0_mesh.obj")
...
def test_batch(test_file,conf):
    ...
    with open("recording_{}.json".format(os.path.basename(test_file)[:-4]), "w") as f:
        json.dump(score_dict,f,indent=4)
    with open("recording_meshudf_{}.json".format(os.path.basename(test_file)[:-4]), "w") as f:
        json.dump(score_dict_meshudf,f,indent=4)
