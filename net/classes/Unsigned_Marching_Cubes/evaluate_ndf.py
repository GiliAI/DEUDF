...
def get_face_angle_op(neighbors, edge_weights):
    ...
    w = w_j * w_k
    return point_j, point_k, w


def get_face_angle_loss(xyz, point_j, point_k, w):
    xyz_j = xyz[point_j]
    xyz_k = xyz[point_k]
    xyz_i = xyz[:].unsqueeze(0)
    dist = xyz_j + xyz_k - 2 * xyz_i
    face_loss = w * torch.norm(dist, dim=2)
...
class Evaluator:
    def __init__(self, args, conf_path, udf_network=None, total_size=None, centers=None):
        ...
        self.lap_iter = self.conf.get_int('evaluate.lap_iter')
        self.use_origin = self.conf.get_int("evaluate.use_origin")
        self.use_edge_loss = self.conf.get_int("evaluate.use_edge_loss")
        self.edg_iter = self.conf.get_int("evaluate.edg_iter")
    ...
    def evaluate(self):
        ...
        query_func = lambda pts: self.udf_network.udf(pts, self.encoding)
        grad_func = lambda pts: self.udf_network.gradient(pts, self.encoding)
        ...
        for it in range(self.max_iter):
            ...
            if it == self.face_angle_step:
                ...
                point_j, point_k, w = get_face_angle_op(neighbors, edge_weights)
            ...
            while head < num_samples:
                sample_subset = xyz[head: min(head + max_batch, num_samples)].unsqueeze(0)
                ...
                laplacian_loss = 10 * laplacian_loss.mean()
                ...
                loss = df_loss+laplacian_loss
                if neighbors is not None:
                    # face_edge_v = torch.sparse.mm(face_edge_op, xyz[:,0:3])
                    # face_edge_v = face_edge_v - xyz[:, 0:3]
                    # face_edge_v = torch.mul(face_edge_v, face_edge_v)
                    # face_edge_v = face_edge_v[head: min(head + max_batch, num_samples)]
                    # face_angle_loss = 1 * torch.sum(face_edge_v, dim=1)
                    # face_angle_loss = 2000 * face_angle_loss.mean()
                    face_angle_loss = get_face_angle_loss(xyz[:, 0:3], point_j, point_k, w).mean()
                    loss = loss + face_angle_loss

                # distance_loss = torch.norm(origin_xyz - xyz, dim=1)
                # distance_loss[distance_loss < 2 * self.threshold] = 0
            ...
            if (it + 1) % self.save_iter == 0:
                ...
                points = xyz.detach().cpu().numpy()
                mesh.vertices = points
                mesh.export(out_dir + '/OP_mesh_{}.obj'.format(it))
        mesh.vertices = points
