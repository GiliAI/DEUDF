...
class SuperviseUdf(Loss):
    ...
    def __call__(self, model_output : dict, model_input: dict):
        ...
        # if sdf > 0:
        #     sdf_constraint = torch.where(gt_sdf == 0, (pred_sdf-gt_sdf).abs(),
        #                                 torch.zeros_like(pred_sdf)) * sdf
        ...
        # # SDF off the surface
        # if inter > 0:
        #     inter_constraint = torch.where((gt_sdf != -1) & (gt_sdf != 0), (pred_sdf-gt_sdf).abs(),
        #                                 torch.zeros_like(pred_sdf)) * inter
        #     result["inter_loss"] = inter_constraint
        # if sdf > 0:
        #     sdf_constraint = (pred_sdf-gt_sdf).abs() * sdf
        #     result["sdf_loss"] = sdf_constraint
        ...
        if 'base' in model_output:
            ...
            # if base_sdf > 0:
            #     base_constraint = torch.where(gt_sdf == 0, (prediction_base-gt_sdf).abs(),
            #                                 torch.zeros_like(prediction_base)) * base_sdf
            #     result['base_sdf_loss'] = base_constraint
            #  # SDF off the surface
            # if base_inter > 0:
            #     inter_constraint = torch.where((gt_sdf != -1) & (gt_sdf != 0), (prediction_base-gt_sdf).abs(),
            #                                 torch.zeros_like(prediction_base))* base_inter
            ...
            # if base_sdf > 0:
            #     base_constraint = (prediction_base - gt_sdf).abs() * base_sdf
            #     result['base_sdf_loss'] = base_constraint
