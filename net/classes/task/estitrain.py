...
class EstiTrain(Task):
    ...
    def _train_phase(self, scheduler):
        ...
        for name, lr in dict(zip(self._param_groups, scheduler.get_lr())).items():
            if lr == 0:
                ...
                # if(name=="residual"):
                #     getattr(self.runner.network, "base").frozeSigma()
    ...
    def __call__(self):
        ...
        if len(self.phase) > 0:
            self._param_groups = []
            for key in self.phase.keys():
                self._param_groups.append(key)
        else:
            self._param_groups = [name for name, _ in self.runner.network.named_children()]


        optimizer = Optim(lr=self.learning_rate, params=[{'params':[p for p in getattr(self.runner.network, name).parameters()]} 
                                                         for name in self._param_groups], betas=(0.1, 0.9),eps=1e-8)
        ...
        while True:
            ...
            for index , model_input in enumerate(dataset):
                it += 1
                ...
                try:
                    model_input["featureSize"] = self.data.featureSize
                ...
                loss, scalar_loss = self.loss(model_output, model_input)

                if not self.runner.perf_test:
                    self.runner.evaluator.train_hook(model_output, scalar_loss)
